...
function renderPokemon(pokemon) {
  ...
  $newDiv.setAttribute('class', 'pokemon-card');
  ...
  $nestDiv.setAttribute('class', 'pokemon-card-text');
  $newDiv.appendChild($nestDiv);

  const $h2 = document.createElement('h2');
  $h2.textContent = pokemon.name;
  $nestDiv.appendChild($h2);
...
for (const pokemon of pokedex) {
  const pokeDom = renderPokemon(pokemon);
  $row.appendChild(pokeDom);
}
